...
namespace BestBefore
{
    ...
    public class BestBefore : MonoBehaviour
    {
        ...
        /// <summary>
        /// [SECONDS] If application is expired, Quit Time defines the time to wait before the application will quit. If Quit Time has a negative value, no freeze effect will be applied
        /// </summary>
        [Tooltip( "[SECONDS] If application is expired, Quit Time defines the time to wait before the application will quit. If Quit Time has a negative value, no freeze effect will be applied" )]
        ...
        void Update()
        {
            ...
            {
                Time.timeScale = 0;
                if ( Time.realtimeSinceStartup - expiredCounter >= FreezeTime )
                {
                    Time.timeScale = originalTimeScale;
                }
            }
            ...
            if ( QuitTime > 0 )
            {
                if ( Time.realtimeSinceStartup - expiredCounter >= QuitTime )
                {
                    Application.Quit();
                }
            }
